...
class Harmonic(processors.Processor):
    ...
    def get_controls(self, amplitudes, harmonic_distribution, f0_hz):
        ...
        if self.normalize_below_nyquist:
            ...
            harmonic_frequencies = core.get_harmonic_frequencies(f0_hz, n_harmonics)
...
class Sinusoidal(processors.Processor):
    ...
    def get_controls(self, amplitudes, frequencies):
        ...
        if self.freq_scale_fn is not None:
            ...
            amplitudes = core.remove_above_nyquist(
                frequencies, amplitudes, self.sample_rate
            )
    ...
    def get_signal(self, amplitudes, frequencies):
        ...
        amplitude_envelopes = core.resample(
            amplitudes, self.n_samples, method=self.amp_resample_method
        )
...
class FilteredNoise(processors.Processor):
    ...
    def get_signal(self, magnitudes):
        ...
        signal = (
            torch.rand(size=[batch_size, self.n_samples], device=magnitudes.device) * 2
            - 1
        )  # uniform in [-1, 1)
...
class GainNoise(processors.Processor):
    ...
    def get_signal(self, gain):
        ...
        signal = (
            torch.rand(size=[batch_size, self.n_samples], device=gain.device) * 2 - 1
        )  # uniform in [-1, 1)

        # Cut audio into frames.
        frame_size = int(np.ceil(self.n_samples / n_frames))
        audio_frames = torch.split(
            signal, frame_size, dim=1
        ...
        if last_frame_size < frame_size:
            ...
            last_frame_padded = torch.nn.functional.pad(
                audio_frames[-1], pad=(0, pad_length)
            )
            audio_frames = audio_frames[:-1] + [last_frame_padded]
        else:
            pad_length = 0

        audio_frames = torch.stack(
            audio_frames, dim=1
...
class VoiceUnvoicedNoise(processors.Processor):
    ...
    def __init__(
        ...
        b, a = scipy.signal.cheby2(
            N=8, rs=60, Wn=self.hp_cutoff, btype="highpass", fs=16000
        )
        self.a = a
        self.b = b
    ...
    def get_signal(self, noise_gain, voiced_unvoiced):
        ...
        white_noise = (
            torch.rand(size=[batch_size, self.n_samples], device=device) * 2 - 1
        )  # uniform in [-1, 1[

        # Pad audio so that frames can slide like in tf.stft(pad=True) and last frame is complete
        hop_size = (
            self.frame_length // 2
        )  # 50 % overlap is assumed by default for window COLA
        ...
        white_noise = white_noise[
            :, None, None, :
        ]  # add a channel dim and a spatial dim for torch.unfold (requires 4D input)
        white_noise_frames = torch.nn.functional.unfold(
            white_noise, kernel_size=(1, self.frame_length), stride=(1, hop_size)
        )  # [batch_size, frame_size, n_frames]
        white_noise_frames = white_noise_frames.transpose(
            1, 2
        )  # [batch_size, n_frames, frame_size]
        ...
        hp_noise_frames = (
            np.random.rand(batch_size, n_audio_frames, frame_size) * 2 - 1
        )  # uniform in [-1, 1[
        hp_noise_frames = scipy.signal.lfilter(self.b, self.a, hp_noise_frames, axis=-1)
        hp_noise_frames = torch.tensor(
            hp_noise_frames, dtype=torch.float32, device=device
        )

        #  use high-pass filtered noise in voiced frames, white noise in unvoiced frames
        noise_frames = torch.where(
            voiced_unvoiced > 0, hp_noise_frames, white_noise_frames
        )
        ...
        hann_window = torch.hann_window(
            self.frame_length, periodic=True, device=device
        )[None, None, :]
        ...
        noise_frames[:, 0, hop_size:] = (
            noise_frames[:, 0, hop_size:] * hann_window[:, :, hop_size:]
        )

        # overlap add back together
        noise_frames = torch.transpose(
            noise_frames, 1, 2
        )  # [batch_size, frame_size, n_frames]
...
class VoiceUnvoicedNoise2(processors.Processor):
    ...
    def __init__(
        self, n_samples=64000, frame_length=512, initial_bias=0.0, n_magnitudes=40
    ):
    ...
    def get_controls(self, gain, magnitudes, voiced_unvoiced):
        ...
        """
        # Scale the gain.
        gain = core.exp_sigmoid(gain + self.initial_bias, max_value=1.0)

        magnitudes = core.exp_sigmoid(magnitudes * 2.5)

        return {
            "noise_gain": gain,
            "voiced_noise_magnitudes": magnitudes,
    ...
    def get_signal(self, noise_gain, voiced_noise_magnitudes, voiced_unvoiced):
        ...
        voiced_noise = (
            torch.rand(size=white_noise.shape, device=device) * 2 - 1
        )  # uniform in [-1, 1[
        voiced_noise = core.frequency_filter(
            voiced_noise[:, 0, 0, :], voiced_noise_magnitudes
        )[:, None, None, :]
        voiced_noise_frames = torch.nn.functional.unfold(
            voiced_noise, kernel_size=(1, self.frame_length), stride=(1, hop_size)
        )  # [batch_size, frame_size, n_frames]
        voiced_noise_frames = voiced_noise_frames.transpose(
            1, 2
        )  # [batch_size, n_frames, frame_size]

        #  use filtered noise in voiced frames, white noise in unvoiced frames
        noise_frames = torch.where(
            voiced_unvoiced > 0, voiced_noise_frames, white_noise_frames
        )
...
class SourceFilterSynth(processors.Processor):
    ...
    def __init__(
        ...
        self.harmonic_synth = Harmonic(
            n_samples,
            sample_rate,
            scale_fn_distribution=harmonic_distribution_activation,
        )
        if filtered_noise:
            self.noise_synth = FilteredNoise(n_samples)
        else:
            self.noise_synth = GainNoise(n_samples)
    ...
    def get_controls(
        ...
        # harmonic model
        harmonics_control_dict = self.harmonic_synth.get_controls(
            harmonic_amplitudes, harmonic_distribution, f0_hz
        )
        ...
        return {
            "harmonic_amplitudes": harmonics_control_dict["amplitudes"],
    ...
    def get_signal(
        ...
        harmonics = self.harmonic_synth.get_signal(
            harmonic_amplitudes, harmonic_distribution, f0_hz
        )
        noise = self.noise_synth.get_signal(noise_control)
        source = harmonics + noise
        signal = core.filter_with_all_pole(
            source, reflection_coeff, self.audio_frame_size, parallel=self.parallel_iir
        )
        return signal
...
class SourceFilterSynthLSF(processors.Processor):
    ...
    def get_signal(
        ...
        signal = core.apply_all_pole_filter(
            source, filter_coeff, self.audio_frame_size, parallel=self.parallel_iir
        )
        return signal
...
class SourceFilterSynth2(processors.Processor):
    ...
    def __init__(
        ...
        self.harmonic_synth = Harmonic(
            n_samples, sample_rate, scale_fn_distribution=core.exp_sigmoid
        )

        if estimate_voiced_noise_mag:
            self.noise_synth = VoiceUnvoicedNoise2(
                n_samples, audio_frame_size, n_magnitudes=40
            )
        else:
            self.noise_synth = VoiceUnvoicedNoise(
                n_samples, audio_frame_size, hp_cutoff=hp_cutoff
            )
    ...
    def get_signal(
        ...
        **kwargs
        ...
        filter_mag = core.slope_frequency_response(
            harmonics_roll_off, n_freqs=65, f_ref=self.f_ref
        )
        ...
        signal = core.apply_all_pole_filter(
            source, filter_coeff, self.audio_frame_size, parallel=True
        )
        return signal
