...
from torchddsp.core import safe_log, pad_for_stft
from torchddsp.core import torch_float32
...
def compute_mel(
    ...
    mag = compute_mag(
        audio, fft_size, overlap, pad_end, center=False, add_in_sqrt=add_in_sqrt
    )

    mel = torchaudio.transforms.MelScale(
        n_mels=bins, sample_rate=sr, f_min=lo_hz, f_max=hi_hz
    ).to(mag.device)(mag)
    return mel
...
def compute_logmel(
    ...
    mel = compute_mel(
        audio, sr, lo_hz, hi_hz, bins, fft_size, overlap, pad_end, add_in_sqrt
    )
    return safe_log(mel)


def compute_logmag(audio, fft_size=2048, overlap=0.75, pad_end=True):
...
def diff(x, axis=-1):
    ...
    if axis >= len(shape):
        raise ValueError(
            "Invalid axis index: %d for tensor with only %d axes." % (axis, len(shape))
        )
...
def slice(input, begin, size):
    ...
    if dims == 1:
        ...
        slice = input[
            ...
            begin[1] : begin[1] + size[1],
            begin[2] : begin[2] + size[2],
            begin[3] : begin[3] + size[3],
        ]
    else:
        raise NotImplementedError(
            "slice does not support more than 4 dimensions at the moment"
        )
    return slice
