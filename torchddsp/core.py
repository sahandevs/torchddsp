...
def lsf_activation(neural_network_output: torch.Tensor):
    ...
    x = (
        x / torch.sum(x, dim=-1, keepdim=True) * pi
    )  # make the sum along last dim equal to pi
...
def get_harmonic_frequencies(
    frequencies: torch.Tensor, n_harmonics: int
) -> torch.Tensor:
...
def resample(
    ...
    if method == "window":
        ...
        outputs = outputs.permute(
            0, 2, 1, 3
        )  # [batch_size, n_channels, n_frames, optional]
        ...
        outputs = outputs.permute(
            0, 2, 1, 3
        )  # [batch_size, n_frames, n_channels, optional]
...
def frequency_impulse_response(
    magnitudes: torch.Tensor, window_size: int = 0
) -> torch.Tensor:
    ...
    magnitudes = magnitudes.unsqueeze(
        -1
...
def fft_convolve(
    ...
    """
    audio, impulse_response = audio.type(torch.float32), impulse_response.type(
        torch.float32
    )
...
def cross_fade_time_varying_fir(
    ...
    convolved_frame_size = (
        audio_frame_size + ir_size - 1
    )  # signal length after convolution
...
def fft_convolve_windowed(
    ...
    audio_frames = torch.nn.functional.unfold(
        audio, kernel_size=(1, audio_frame_size), stride=(1, hop_size)
    )  # [batch_size, frame_size, n_frames]
    ...
    audio_frames_out = audio_frames_out.transpose(
        1, 2
    )  # [batch_size, fft_size, n_frames]

    audio_out_size = (
        n_ir_frames - 1
    ) * hop_size + fft_size  # time domain length after frame-wise convolution
...
def filter_with_all_pole(
    ...
    filter_coefficients = reflection_to_filter_coeff(
        reflection_coeff
    )  # [batch_size, n_frames, n_coeff]
    ...
    filtered_audio = apply_all_pole_filter(
        audio, filter_coefficients, audio_block_size, parallel=parallel
    )
...
def reflection_to_filter_coeff(reflection_coeff: torch.Tensor):
    ...
    for i in range(n_coeff):
        ...
        if i > 0:
            for j in range(i):
                coeff_for_order_i[:, :, j] = (
                    coeff_for_order_i_minus_1[:, :, j]
                    - reflection_coeff[:, :, i]
                    * coeff_for_order_i_minus_1[:, :, i - j - 1]
                )
...
def lsf_to_filter_coeff(line_spectral_frequencies: torch.Tensor):
    ...
    p_q_prime = torch.zeros(
        (batch_size, n_frames, 2, filter_order // 2 + 3),
        device=device,
        dtype=torch.float64,
    )  # k = -2, -1, 0, 1, ..., filter_order/2
    ...
    p_q = torch.zeros(
        (batch_size, n_frames, 2, filter_order // 2), device=device, dtype=torch.float64
    )
    for k in range(0, filter_order // 2):
        p_q[:, :, 0, k] = p_q_prime[:, :, 0, k + 3] + p_q_prime[:, :, 0, k + 2]
        p_q[:, :, 1, k] = p_q_prime[:, :, 1, k + 3] - p_q_prime[:, :, 1, k + 2]

    filter_coefficients = torch.zeros(
        (batch_size, n_frames, filter_order), device=device, dtype=torch.float64
    )

    for k in range(0, filter_order // 2):
        filter_coefficients[:, :, k] = 0.5 * (p_q[:, :, 0, k] + p_q[:, :, 1, k])
        filter_coefficients[:, :, filter_order // 2 + k] = 0.5 * (
            p_q[:, :, 0, filter_order // 2 - k - 1]
            - p_q[:, :, 1, filter_order // 2 - k - 1]
        )
...
def apply_all_pole_filter(
    ...
    if batch_size != batch_size_filter:
        raise ValueError(
            "Batch size of audio ({}) and filter ({}) must "
    ...
    audio_frames = torch.nn.functional.unfold(
        audio, kernel_size=(1, audio_block_size), stride=(1, hop_size)
    )  # [batch_size, frame_size, n_frames]
    audio_frames = audio_frames.transpose(1, 2)  # [batch_size, n_frames, frame_size]
    ...
    filtered_audio = torch.zeros(
        (batch_size, n_audio_frames, audio_block_size + n_coeff), device=device
    )
    ...
    if parallel:
        for n in range(audio_block_size):
            filtered_audio[:, :, n + n_coeff] = audio_frames[:, :, n] - torch.sum(
                filter_coeff * filtered_audio[:, :, n : n + n_coeff].clone(), dim=-1
            )
    else:
        ...
        for n_f in range(n_audio_frames):
            if n_f > 0:
                ...
                filtered_audio[:, n_f, :n_coeff] = filtered_audio[
                    :, n_f - 1, hop_size : hop_size + n_coeff
                ]
            for n in range(audio_block_size):
                # index n_coeff should be interpreted as index 0 in filtered_audio
                filtered_audio[:, n_f, n + n_coeff] = audio_frames[
                    :, n_f, n
                ] - torch.sum(
                    filter_coeff[:, n_f, :]
                    * filtered_audio[:, n_f, n : n + n_coeff].clone(),
                    dim=-1,
                )
    ...
    hann_window = torch.hann_window(audio_block_size, periodic=True, device=device)[
        None, None, :
    ]
    ...
    filtered_audio[:, 0, hop_size:] = (
        filtered_audio[:, 0, hop_size:] * hann_window[:, :, hop_size:]
    )

    filtered_audio = torch.transpose(
        filtered_audio, 1, 2
    )  # [batch_size, frame_size, n_frames]
    ...
    audio_out_size = (
        n_audio_frames - 1
    ) * hop_size + audio_block_size  # time domain length after frame-wise filtering
    audio_out = torch.nn.functional.fold(
        filtered_audio,
        output_size=(1, audio_out_size),
...
def frequencies_sigmoid(
    freqs: torch.Tensor, depth: int = 1, hz_min: float = 0.0, hz_max: float = 8000.0
) -> torch.Tensor:
...
def hz_to_unit(
    ...
    midi = hz_to_midi(hz)
    return midi_to_unit(
        midi, midi_min=hz_to_midi(hz_min), midi_max=hz_to_midi(hz_max), clip=clip
    )
...
def minus_12db_per_octave_distribution(freqs, f_ref):
    ...
    amplitude_factors = torch.where(
        freqs > f_ref,
        a_0 ** torch.log2(freqs / f_ref),
        torch.tensor(1.0, device=freqs.device),
    )
