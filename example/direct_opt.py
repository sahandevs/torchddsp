...
class DirectOptimization(torch.nn.Module):
    def __init__(self, estimate_f0=True):
        ...
        self.source_filter_synth = synths.SourceFilterSynth2(
            ...
            hp_cutoff=500,
            f_ref=torch.tensor(200, dtype=torch.float32),  # type: ignore
            estimate_voiced_noise_mag=True,
        )
        self.harmonic_amp_params = torch.nn.Parameter(
            torch.ones(n_sources, n_frames, 1)
        )
        self.harmonic_roll_off_params = torch.nn.Parameter(
            torch.ones(n_sources, n_frames, 1)
        )
    ...
    def forward(self, f0_hz=None):
        ...
        if self.estimate_f0:
            f0_hz = self.f0_p
        f0_hz = f0_hz.transpose(  # type: ignore
            1, 2
